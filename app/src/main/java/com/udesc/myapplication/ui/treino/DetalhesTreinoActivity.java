...
public class DetalhesTreinoActivity extends AppCompatActivity {
        ...
        RetrofitClient.getApiService().treinoPorId(treinoId).enqueue(new Callback<TreinoDTO>() {
            ...
            public void onResponse(@NonNull Call<TreinoDTO> call, @NonNull Response<TreinoDTO> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Toast.makeText(DetalhesTreinoActivity.this, "Erro ao carregar detalhes", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(@NonNull Call<TreinoDTO> call, @NonNull Throwable t) {
                ...
                Toast.makeText(DetalhesTreinoActivity.this, "Erro de conexão: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }

    private void exibirDetalhesTreino(TreinoDTO treino) {
        titleView.setText(treino.getNomeTreino());
        
        if (treino.getDataCriacao() != null) {
            dateView.setText(DateHelpers.format(treino.getDataCriacao()));
        }
    ...
    private View criarViewSerie(ExercicioSerieDTO serie) {
